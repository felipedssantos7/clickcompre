...
if($client->get_name() != '' 
    ...
    if(strlen(($client->get_name())) <= 2){
        ...
        header('Location: client_register_page.php?invalide_datas=2');
        die();
    } else if(!filter_var($client->get_email(), FILTER_VALIDATE_EMAIL)){
        header('Location: client_register_page.php?invalide_datas=3');
        die();
    } else if(strlen($client->get_password()) < 6){
        header('Location: client_register_page.php?invalide_datas=4');
        die();
    }
} else {
    # Retorna para página de cadastro.
    header('Location: client_register_page.php?invalide_datas=5');
    die();
}
